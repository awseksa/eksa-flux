...
{{- define "cnfTplMetadata_2_27" -}}
{{- include "metaSpec_2_27" (merge (dict "specOffset" (add $.setOffset 0 | int) "metaSpec" $.metadata) (dict "nfVariables" $.cnfHdr.nfVariables)) -}}
{{- end -}}
...
{{- include "k8sLabels_2_27" (merge (dict "specOffset" (add $specOffset 4 | int)) (dict "nfVariables" .nfVariables)) -}}
{{- if $custom_extensions.labels -}}
{{- include "customLabels" (merge (dict "specOffset" (add $specOffset 4 | int)) (dict "nfVariables" .nfVariables)) -}}
{{- end -}}
{{- if and ( or (eq (.nfVariables.configtype|toString) "mgmt-cfg") (eq (.nfVariables.configtype|toString) "static-cfg")) (.nfVariables.filepath) -}}
{{- $multipath := get $.nfVariables.root.Values.nf.cnftpl_configmap (.nfVariables.configtype| replace "-" "_" | toString) }}
{{- if (hasKey $multipath (.nfVariables.filepath|toString)) -}}
{{- $multipath := get $multipath (.nfVariables.filepath|toString) }}
{{- if $multipath.metaspec -}}
{{- $multipath := get $multipath "metaspec" }}
{{- if and .metaSpec.labels $multipath.labels -}}
{{- $multipath := mergeOverwrite .metaSpec.labels $multipath.labels -}}
{{- range $key, $value := $multipath }}
{{- include "eHdr" (dict "key" $key "value" ($value | quote) "hdrOffset" (add $specOffset 4 | int)) -}}
{{- end -}}
{{- else if and (not .metaSpec.labels) $multipath.labels -}}
{{- $multipath := $multipath.labels -}}
{{- range $key, $value := $multipath }}
{{- include "eHdr" (dict "key" $key "value" ($value | quote) "hdrOffset" (add $specOffset 4 | int)) -}}
{{- end -}}
{{- else -}}
...
{{- include "tmaasSpec_2_27" (merge (dict "specOffset" (add $specOffset 4 | int)) (dict "nfVariables" .nfVariables)) -}}
...
{{- if $multipath.metaspec -}}
{{- $multipath := get $multipath "metaspec" }}
{{- if and .metaSpec.annotations $multipath.annotations -}}
{{- $multipath := mergeOverwrite .metaSpec.annotations $multipath.annotations -}}
{{- range $key, $value := $multipath }}
{{- include "eHdr" (dict "key" $key "value" ($value | quote) "hdrOffset" (add $specOffset 4 | int)) -}}
{{- end -}}
{{- else if and (not .metaSpec.annotations) $multipath.annotations -}}
{{- $multipath := $multipath.annotations -}}
{{- range $key, $value := $multipath }}
{{- include "eHdr" (dict "key" $key "value" ($value | quote) "hdrOffset" (add $specOffset 4 | int)) -}}
{{- end -}}
{{- else -}}
...
{{- if ne (or (hasSuffix ".tgz" $path) (hasSuffix ".gz" $path) (hasSuffix ".bz2" $path)) true -}}
...
{{- define "configMapBinaryDataSpec_2_27" -}}
{{- $specOffset := .specOffset -}}
{{- $nfVariables := $.nfVariables -}}
...
{{- end -}}
{{- if eq ($.nfVariables.configtype|toString) "mgmt-cfg" -}}
{{- $mgmtContents := printf "%s" ($.nfVariables.mgmtbinpathformatgz|toString) -}}
{{- range $path, $_ := ($.nfVariables.root.Files.Glob ($mgmtContents|toString)) -}}
{{- $path | base | replace "@" "_rev_" | trimSuffix "\n" | nindent (add $specOffset 2 | int) }}:
{{- ($.nfVariables.root.Files.Get ( $path |toString)) | b64enc | trimSuffix "\n" | nindent (add $specOffset 4 | int) }}
{{- end -}}
{{- end -}}
{{- if eq ($.nfVariables.configtype|toString) "mgmt-cfg" -}}
{{- $mgmtContents := printf "%s" ($.nfVariables.mgmtbinpathformatbz|toString) -}}
{{- range $path, $_ := ($.nfVariables.root.Files.Glob ($mgmtContents|toString)) -}}
{{- $path | base | replace "@" "_rev_" | trimSuffix "\n" | nindent (add $specOffset 2 | int) }}:
{{- ($.nfVariables.root.Files.Get ( $path |toString)) | b64enc | trimSuffix "\n" | nindent (add $specOffset 4 | int) }}
{{- end -}}
